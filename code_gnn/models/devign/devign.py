...
class DevignModule(BaseModule):
    def __init__(self, input_dim, window_size, graph_embed_size, num_layers, **kwargs):
        ...
        if graph_embed_size >= total_input_dim:
            ...
            print(
                f"Creating downsample layer from {total_input_dim} to {ggnn_input_dim}"
            )
            self.downsample_layer = torch.nn.Linear(total_input_dim, ggnn_input_dim)

        # construct neural network layers
        self.ggnn = GatedGraphConv(
            in_feats=ggnn_input_dim,
            out_feats=graph_embed_size,
            n_steps=num_layers,
            n_etypes=n_etypes,
        )
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=100,
            help="(should be renamed) vector size for Word2Vec",
        )
        parser.add_argument(
            "--graph_embed_size",
            type=int,
            default=169,
            help="width of the GNN node embedding",
        )
        parser.add_argument(
