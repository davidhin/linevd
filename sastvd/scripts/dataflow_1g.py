    ...
    if cache_file.exists() and not force:
        ...
        if cache_file_funcs.exists() and not force_func:
            df = pd.read_csv(
                cache_file_funcs,
                index_col=0,
                converters={"graph_id": int, "summary_filepath": str, "function": str},
            )
            ...
            df["function"] = svd.dfmp(
                df, load_summary, "summary_filepath", workers=workers, desc="function"
            )
        ...
        def load_func(row):
            func = row["function"]
            with open(
                str(row["summary_filepath"]).replace(
                    ".dataflow.summary.json", ".dataflow." + func + ".json"
                )
            ) as f:
                kg = json.load(f)
        ...
        df["genkill"] = svd.dfmp(
            df,
            load_func,
            ["graph_id", "function", "summary_filepath"],
            workers=workers,
            desc="genkill",
        )
        ...
        df = pd.concat(
            (v for _, v in df["genkill"].apply(pd.DataFrame).iteritems()),
            ignore_index=True,
        )
        print("joined", df)
