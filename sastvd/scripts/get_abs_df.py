...
def expand_struct_datatypes_input(df):
    ...
    md_df["filepath"] = md_df.apply(lambda row: checkout_dir/(row["repo"].replace("://", "__").replace("/", "__") + "__" + row["commit_id"]), axis=1)
    md_df = md_df[md_df["filepath"].apply(lambda p: p is not None and p.exists())]
    df = pd.merge(df, md_df, left_on="graph_id", right_on="id")
    df = df.drop_duplicates(subset=["repo", "commit_id", "datatype"]).sort_values("filepath")

    df = (
        df[["node_id","graph_id","project_x","codeLink","repo","commit_id","filepath","datatype","operator","api","literal"]]
        .rename(columns={"project_x": "project"})
        )

    df["datatype_subtypes"] = pd.NA
    df["datatype"] = df["datatype"].apply(lambda dt: dt if pd.isna(dt) else (
        re.sub(r"(\*|\.)", r"\\\1",                     # escape datatype tokensregular expressions
            re.sub(r"^struct ", r"",                    # exclude struct keyword
                re.sub(r"(\s*\*)+\s*$", r"",            # exclude pointer declaration
                    re.sub(r"^const ", r"",             # exclude const keyword
                        re.sub(r"\s+\[.*\]", r"", dt)   # exclude brackets for arrays
                    )
                )
            )
        )
...
def run_joern_adapter(df, worker_id, n_splits, test):
    ...
    if test == "test":
        ...
        save_file = Path(f"bigvul_metadata_with_commit_id_slim_with_subtypes_{worker_id}.csv")
...
def run_joern(df, worker_id, save_file, cache):
    sess = svdjs.JoernSession(f"datatype/{worker_id}", logfile=open(f"repos/logs/get_abs_df_{worker_id}.txt", "wb"))
    ...
    try:
        for filepath, group in tqdm.tqdm(df.groupby("filepath"), desc="load types"):
            ...
            try:
                ...
                print("ids", group["graph_id"].sort_values().unique().tolist(), "filepath", filepath, "extracted subtypes for", len(dt_to_subtypes), "/", len(group), "datatypes")
                for i, row in group.iterrows():
                    ...
                    if dt in dt_to_subtypes:
                        ...
                        print("graph_id", row["graph_id"], "id", i, "-", dt, "has no subtypes")
    ...
    df = df.assign(datatype_subtypes_str=df["datatype_subtypes"].apply(lambda st: ", ".join(sorted(st))))
    tdf = df.assign(datatype_subtypes=df["datatype_subtypes"].apply(lambda st: json.dumps(st)))
...
if __name__ == "__main__":
    ...
    expanded_dataflow_df = run_joern_adapter(expanded_dataflow_df_input, worker_id, n_splits, test)
