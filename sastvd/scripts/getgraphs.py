...
def preprocess(row, run_sast=True):
    ...
    if row["dataset"] == "sard":
        ...
        # Run Joern on code
        if not os.path.exists(f"{fpath}.edges.json"):
            svdj.full_run_joern(fpath, verbose=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_sast", action="store_false")
    ...
    svd.dfmp(splits[JOB_ARRAY_NUMBER], preprocess, ordr=False, workers=args.workers)
